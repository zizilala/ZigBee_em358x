...
static int32u currentOffset;
static int8u blockNum;
static int8u retries;
...
static void resetState(boolean success)
{
  (*finishedFunction)(success);
  xmodemSendState = XMODEM_SEND_STATE_NONE;
}

static EmberStatus sendNextBlock(boolean success)
{
  int8u returnLength;
  EmberStatus status;
  int8u dataBuffer[XMODEM_MAX_DATA_SIZE + XMODEM_OVERHEAD];
  boolean sendEot = (xmodemSendState == XMODEM_SEND_STATE_WAITING_EOT_ACK);
