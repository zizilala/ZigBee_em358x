...
#define IRQx_PRIORITY_REGISTER(irq) \
  (* ( ((int8u *)NVIC_IPR_3to0_ADDR) + EVAL3(IRQ,irq,_VECTOR_INDEX) - 16) )
...
static boolean halExtDevicePowered = FALSE;
static boolean halExtDeviceSelected = FALSE;
static int8u halExtDeviceIntLevel = EXT_DEVICE_INT_UNCONFIGURED;
 ...
 */
boolean halExtDeviceIsReady(void)
{
  #ifdef  EXT_DEVICE_RDY
 ...
 */
boolean halExtDeviceIsSelected(void)
{
 ...
 */
boolean halExtDeviceIntPending(void)
{
  #ifdef  EXT_DEVICE_INT
 ...
 */
HalExtDeviceIntLevel halExtDeviceIntEnable(boolean clearPending)
{
  #ifdef  EXT_DEVICE_INT
    int8u origLevel;
    boolean justEnabled = FALSE;
    ATOMIC( // ATOMIC because these routines might be called from other ISRs
      origLevel = halExtDeviceIntLevel;
      if (origLevel != EXT_DEVICE_INT_UNCONFIGURED) {
