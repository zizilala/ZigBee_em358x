...
static EmberAfStatus returnStatus(boolean wasHandled, boolean mfgSpecific);
...
static boolean checkKeyUsage(int8u keyUsageByte)
{
  if(isCbkeKeyEstablishmentSuite283k1()){
    if(!(keyUsageByte & KEY_USAGE_BYTE_VALID_VALUE)){
      emberAfKeyEstablishmentClusterPrintln("Error: Invalid Key Usage Byte %u",keyUsageByte);
  ...
  case BEGIN_KEY_ESTABLISHMENT:
    {
      ...
      if (partner.isInitiator) {
        if (!checkRequestedSuite(data1)
            ...
            || !checkKeyUsage(*(data2 + KEY_USAGE_BYTE))) {
  ...
  case SEND_CONFIRM_KEY_MESSAGE:
    {
      ...
      int8u *printInitiator = emberSmacContents(initiatorSmac);
      int8u *printResponder = emberSmacContents(responderSmac);
...
//Helper function copied from call-command-handler.c
static EmberAfStatus returnStatus(boolean wasHandled, boolean mfgSpecific)
{
  if (wasHandled) {
...
static EmberAfStatus validateNewCbkeSuite(int8u endpoint,
                                          int16u newCbkeSuite)
{
  if(newCbkeSuite >= EMBER_AF_INVALID_KEY_ESTABLISHMENT_SUITE &&
     newCbkeSuite <= emberAfIsFullSmartEnergySecurityPresent()){
                                                                              ...
                                                                              int8u *value)
{
  switch(attributeId){
    case ZCL_KEY_ESTABLISHMENT_SUITE_SERVER_ATTRIBUTE_ID:
      {
        int16u newCbkeSuite;
        MEMCOPY(&newCbkeSuite,value,size);
