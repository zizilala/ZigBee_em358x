...
boolean bootloadForceActivation( void )
{
    ...
    halGpioConfig(BUTTON_RECOVERY_PORT_PIN, GPIOCFG_OUT);
    BUTTON_RECOVERY_SET();
    for(i=0; i<100; i++)
      __no_operation();

    // Reconfigure as an input with pullup to read the button state
    halGpioConfig(BUTTON_RECOVERY_PORT_PIN, GPIOCFG_IN_PUD);
    ...
    halGpioConfig(BUTTON_RECOVERY_PORT_PIN, GPIOCFG_IN);
