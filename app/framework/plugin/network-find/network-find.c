...
// DCS code to compliantly search for all channels once we've searched on the
// preferred channels.  
EmberStatus emberAfStartSearchForJoinableNetworkAllChannels(void)
{
  ...
  emberAfAppPrint("Search for %p\nChannels: ", "joinable network");
  emberAfAppDebugExec(emberAfPrintChannelListFromMask(EMBER_ALL_802_15_4_CHANNELS_MASK));
  emberAfAppPrintln("");

  emberAfGetFormAndJoinExtendedPanIdCallback(extendedPanId);
  status = emberScanForJoinableNetwork(EMBER_ALL_802_15_4_CHANNELS_MASK, extendedPanId);
  if (status == EMBER_SUCCESS) {
    state = NETWORK_FIND_JOIN;
  }
  return status;
